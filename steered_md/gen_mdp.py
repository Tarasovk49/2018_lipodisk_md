...
def gen_mdp(input_mdp, index, output_mdp):
    ...
    with open(output_mdp,'w') as mdp:
        with open(input_mdp) as main_mdp:
            ...
            mdp.write('; Pull code'+'\n')
            mdp.write('pull                    = yes'+'\n')
            mdp.write('pull_ngroups            = {0}'.format(sma_mol_count+1)+'\n')
            mdp.write('pull_ncoords            = {0}'.format(sma_mol_count)+'\n')
            mdp.write('pull_group1_name        = MEMBRANE'+'\n')
            for i in range(1,1+sma_mol_count):
                ...
                mdp.write('pull_coord{0}_init        = 4             ; distance decreases while it is higher than 4 nm'.format(i)+'\n')
